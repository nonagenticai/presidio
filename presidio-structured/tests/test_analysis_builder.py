...
def test_find_mixed_strategy_entity_with_custom_mixed_strategy_threshold(
    tabular_analysis_builder, sample_df
):
    key_recognizer_result_map = tabular_analysis_builder._generate_key_rec_results_map(
...
def test_find_entity_with_invalid_strategy_raises_exception(
    ...
    assert f"Unsupported entity selection strategy: {selection_strategy}." in str(
        excinfo.value
    )
...
def test_analysis_tabular_when_multiprocess_then_results_are_correct(
    ...
    analyzer_engine = AnalyzerEngine(default_score_threshold=0)
    tabular_analysis_builder = PandasAnalysisBuilder(
        analyzer_engine, n_process=4, batch_size=2
    )
    structured_analysis = tabular_analysis_builder.generate_analysis(sample_df)
...
def test_analysis_json_when_default_threshold_is_high_then_only_email_passes(
    ...
    analyzer_engine = AnalyzerEngine(
        default_score_threshold=0.9, log_decision_process=True
    )
