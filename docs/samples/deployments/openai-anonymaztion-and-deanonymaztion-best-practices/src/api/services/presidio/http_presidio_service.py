...
class HttpPresidioService(PresidioService):
    ...
    def anonymize_text(
        ...
        try:
            ...
            analyze_payload = {"text": text, "language": language}
            ...
            analyzer_result_with_id, entities = self.add_id_to_analyzer_result(
                text, analyzer_result, entity_mappings
            )
            ...
            new_entity_mappings = self.build_entity_mappgings(
                entity_mappings, entities, anonymizer_results
            )
    ...
    def build_entity_mappgings(
        self, entity_mappings: dict, entities: dict, anonymizer_results: List
    ) -> dict:
    ...
    def add_id_to_analyzer_result(
        self, text: str, analyzer_result: List[OperatorResult], entity_mappings: dict
    ) -> Tuple[List[OperatorResult], dict]:
